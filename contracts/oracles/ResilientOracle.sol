...
/**
 * @dev Updated from https://github.com/VenusProtocol/oracle/blob/develop/contracts/ResilientOracle.sol
 */
contract ResilientOracle is ReentrancyGuard, Pausable, ProtocolOwner, OracleInterface {
  ...
  ) notNullAddress(address(_boundValidator)) ProtocolOwner(_protocol) {
  ...
  ) public notNullAddress(tokenConfig.asset) notNullAddress(tokenConfig.oracles[uint256(OracleRole.MAIN)]) onlyOwner {
  ...
  function setOracle(
    ...
    address oracle,
    OracleRole role
  ) external notNullAddress(asset) checkTokenConfigExistence(asset) onlyOwner {
  ...
  function _getPrice(address asset) internal view returns (uint256) {
    ...
    // console.log('_getPrice, pivotPrice: %s, pivotOracleEnabled: %s', pivotPrice, pivotOracleEnabled);
    ...
    // console.log('_getPrice, mainPrice: %s, validatedPivotMain: %s', mainPrice, validatedPivotMain);
